...
class TemplateWizard(QtWidgets.QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        crop_auto_label = QtWidgets.QLabel(
            "Автоматически: фото заполняет область целиком (лишнее по краям обрезается)")
    ...
    def _update_hint(self) -> None:
        ...
        if self._mode == "photo":
            self.hint_label.setText(
                ...
                "растягивает область. Фото автоматически заполняет область "
                "целиком (края совпадают с границами области, лишнее по краям "
                "обрезается) — выберите лишь «Масштаб по лицу» в панели справа. "
                "Готовый бейдж смотрите на вкладке «Пример бейджа».")
        else:
            field = self._current_field()
            if field is not None:
                if field.id == "position":
                    self.hint_label.setText(
                        "Поле «Должность»: кликните по макету — это ЛЕВЫЙ край "
                        "области текста. Ширина области задаётся полем "
                        "«Макс. ширина» справа, текст выравнивается по центру "
                        "этой области. Готовый бейдж смотрите на вкладке "
                        "«Пример бейджа».")
                else:
                    self.hint_label.setText(
